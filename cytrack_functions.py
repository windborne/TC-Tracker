...
def print_dafault_values(cyclone_type="",
	...
	if  str2boolean(verbose):
		...
		print("+ Terrain data path: {0}".format(path_data_terr_source))
		print("+ Single level data path: {0}".format(path_data_source))
		print("+ Pressure level data path: {0}".format(path_data_source_upper))
...
def get_era5_files(dates=[""],hours=[""],era_file_prefix="",era_date_file_name="yyyymmdd_hh"):
	...
	return erafiles
def get_wm_files(dates=[""],hours=[""],wm_file_prefix="",wm_date_file_name="yyyymmdd_hh",begin_year='2024',begin_month='06',begin_day='24',begin_hour='00',mslp_anom_files=False,path_data_source="",path_data_source_upper=""):
	wmfiles=[]
	for i in range(0,len(dates)):
		year=dates[i][0:4].zfill(4)
		month=dates[i][4:6].zfill(2)
		...
		if wm_date_file_name=="yyyymmdd_hh":
			nfame=year+month+day+"_"+hour
		elif wm_date_file_name=="yyyymmdd_hh":
			nfame=year+month+day+hour
		else:
...
def get_mslp_anomaly(idir="./",
	...
	if source.upper()=="WM":
		previous_files=get_wm_files(dates=dates,hours=hours,wm_file_prefix=source_filename_prefix,wm_date_file_name=source_file_date_format,begin_year=begin_year,begin_month=begin_month,begin_day=begin_day,begin_hour=begin_hour,mslp_anom_files=True,path_data_source=path_data_source,path_data_source_upper=path_data_source_upper)
		mean_slp=0
		for index in range(0,len(previous_files)):
			varlist=get_wm_2dvar_anom(idir=idir,erafile=previous_files[index],svariables=["msl"],search_limits=search_limits,search_region=search_region,path_data_source=path_data_source,path_data_source_upper=path_data_source_upper)
			eramslp=varlist[2,:]/100
			mean_slp=mean_slp+eramslp
...
def get_wm_2dvar_mslp(idir="./",
	...
	if search_region.upper() in ("NA","SA","AL","MS"):
		for i in range(0,len(svariables)):
			vars()[svariables[i]],nlon=convert_wm_matrix(vars()[svariables[i]],eralon,search_lon=180)
...
def convert_wm_matrix(matrix,lon,search_lon):
	index=np.where(lon==search_lon)
	index=int(index[0])
	aux_matrix=np.empty_like(matrix)
	aux_matrix[:,:index]=matrix[:,index:]
...
def get_wind_speed(latsc=[None],lonsc=[None],radius=[None],wfile="",idir="./",varu="U",varv="V",varlat="lat",varlon="lon",source="WRF",search_limits=[None,None,None,None],search_region="",r_uv=False,path_data_source="",path_data_source_upper=""):
	
	print('-----Collecting Wind Speed------')
	if source.upper()=="WRF":
		...
		u=varlist[2,:]
		v=varlist[3,:]
	elif source.upper()=="WM":
		varlist=get_wm_2dvar_wind(idir=idir,erafile=wfile,svariables=[varu,varv],search_limits=search_limits,search_region=search_region,path_data_source=path_data_source,path_data_source_upper=path_data_source_upper)
		wlat=varlist[0,:]
		wlon=varlist[1,:]
		u=varlist[2,:]
		v=varlist[3,:]
...
def tracker_cyclones(cyclone_type="",
	...
	if terrain_filter>0:
		...
		if source.upper()=="WM":
			if not os.path.isfile(path_data_terr_source):
				print_error_message("Terrain data is not found in the specified location => {0}".format(path_data_terr_source))
			varhgt=get_era5_2dvar(idir="",erafile=path_data_terr_source,svariables=["z"],search_limits=search_limits,search_region=search_region)
			hgt_field=varhgt[2,:]/9.80665
		elif source.upper()=="WRF":
	...
	#for index in range(0,8):
		...
		if verbose:
			print("   ---> | Processing "+source.upper()+ ": "+sourcefiles[index])
		if source.upper()=="ERA5":
			varu="u10"
			varv="v10"
			varlat="latitude"
			...
			varlist=get_wm_2dvar_mslp(idir=idir,erafile=sourcefiles[index],svariables=["msl"],search_limits=search_limits,search_region=search_region,path_data_source=path_data_source,path_data_source_upper=path_data_source_upper)
			sourcelat=varlist[0,:]
			sourcelon=varlist[1,:]
			sourcemslp=varlist[2,:]/100
			if index == 0:
		...
		if use_mslp_anomaly:
			...
			avg_mslp=get_mslp_anomaly(idir=idir,
							...
							source_file_date_format='yyyymmdd_hh',
							...
							begin_year=begin_year,begin_month=begin_month,
							begin_day=begin_day,begin_hour=begin_hour,
...
def get_VTU_VTL_B_series(cenlats=np.array([None]),
	...
	if source.upper()=="WM":
		upperfiles=get_wm_files(dates=dates,hours=hours,wm_file_prefix='WM',wm_date_file_name='yyyymmdd_hh',begin_year=begin_year,begin_month=begin_month,begin_day=begin_day,begin_hour=begin_hour,mslp_anom_files=False,path_data_source=path_data_source,path_data_source_upper=path_data_source_upper)
						...
						if os.stat(tmpdir+"/critical_centers_"+nfdate+".dat").st_size == 0:
							if (len(nlats)-1)*dt_h>=dt_lifetime and nmws.max()>intensity_threshold:
								...
								if  checking_upper_levels_parameters:
									print('--------------------------------')
									print(begin_year,begin_month,begin_day)
									print('--------------------------------')
...
def write_tracker(cyclone_type,nlats,nlons,npmin,nmws,nclosedp,nroci,nouter_r,nctype,VTU,VTL, Bhart, search_region,sys_id,dir,ndates, nhours,start_date):
	...
	if cyclone_type.upper() in ("EC"):
		...
		dir["Cy"+search_region+str(sys_id).zfill(4)+ndates[0][0:4]]['last_closed_isobar_value'] = list(nclosedp)
		dir["Cy"+search_region+str(sys_id).zfill(4)+ndates[0][0:4]]['radius_of_outer_isobar'] = list(nroci)
		dir["Cy"+search_region+str(sys_id).zfill(4)+ndates[0][0:4]]['TC_type'] = list(nnctype)
		dir["Cy"+search_region+str(sys_id).zfill(4)+ndates[0][0:4]]['VTU'] = list(VTU)
		dir["Cy"+search_region+str(sys_id).zfill(4)+ndates[0][0:4]]['VTL'] = list(VTL)
		dir["Cy"+search_region+str(sys_id).zfill(4)+ndates[0][0:4]]['Bhart_value'] = list(Bhart)
