...
def get_cytrack_main(pathfile=""):
	...
	wm_file_prefix = check_paths(content, "wm_file_prefix")
	wm_upperfile_prefix = check_paths(content, "wm_upperfile_prefix")
	wm_date_file_name = check_paths(content, "wm_date_file_name")
	use_mslp_anomaly=check_paths(content, "use_mslp_anomaly")
	model_res = check_paths(content, "model_res")
	path_data_terr_source = check_paths(content,"path_data_terr_source")
	...
	two_week_back = datetime(int(begin_year),int(begin_month),int(begin_day),int(begin_hour)) - timedelta(days=14)
	begin_year2 = two_week_back.strftime('%Y');begin_month2 = two_week_back.strftime('%m')
	begin_day2 = two_week_back.strftime('%d');begin_hour2 = two_week_back.strftime('%H')
		
	dates,hours=get_dates_vectors(year_case_init=begin_year2,
		month_case_init=begin_month2,
		day_case_init=begin_day2,
		hour_case_init=begin_hour2,
	...
	if source.upper()=="WRF":
		...
		input_files=get_wm_files(dates=dates,hours=hours,wm_file_prefix=wm_file_prefix,wm_date_file_name=wm_date_file_name,begin_year=begin_year,begin_month=begin_month,begin_day=begin_day,begin_hour=begin_hour,mslp_anom_files=False,path_data_source=path_data_source,path_data_source_upper=path_data_source_upper)
		source_prefix=wm_file_prefix
	...
	elif source.upper()=="WM" and checking_upper_levels_parameters==True:
		input_files_upper=get_wm_files(dates=dates,hours=hours,wm_file_prefix=wm_file_prefix,wm_date_file_name=wm_date_file_name,path_data_source=path_data_source,begin_year=begin_year,begin_month=begin_month,begin_day=begin_day,begin_hour=begin_hour)
		source_upperprefix=wm_upperfile_prefix
